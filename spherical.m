...
% boundary conditions. Notably, the q is given in pure power - note that this is
...
function [x, T] = spherical(h2, bulkTemp2, resistanceInfo, Temp1, xStep)
    ...
    for resistance = 3:size(resistanceInfo, 1) % loops thru rows starting at the third element
        ...
        totalResistance = totalResistance + resistanceInfo(resistance, 1)/(A_GM(r_o, r_i) * resistanceInfo(resistance, 2)); 
    end
